...
public abstract class AbstractTextTransformer : ITransformer
{
    ...
    protected void AppendFooter(TextWriter body, int htmlSize, int gmiSize)
    {
        ...
        body.WriteLine(
            $"Size: {ReadableFileSize(gmiSize)}. {Savings(gmiSize, htmlSize)} smaller than original: {ReadableFileSize(htmlSize)} {emoji}");
        body.WriteLine("=> mailto:acidus@gemi.dev Made with ❤️ by Acidus");
    }

    protected string Savings(int newSize, int originalSize)
    {
        return string.Format("{0:0.00}%", (1.0d - Convert.ToDouble(newSize) / Convert.ToDouble(originalSize)) * 100.0d);
    }
    ...
    protected static Uri CreateUrl(string s)
    {
        ...
        {
            var u = new Uri(s);
            if (!u.IsAbsoluteUri || u.Scheme == "file") return null;
            return u;
        }
        catch (Exception)
        {
